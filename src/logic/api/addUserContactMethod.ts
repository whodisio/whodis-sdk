...
export const addUserContactMethod = async (
  ...
  try {
    const { data } = await axios.post(`${WHODIS_API_HOST}/sdk/user/contact-method/add`, args, { headers: { authorization: `HMAC ${signature}` } });
    return data.user;
  } catch (error) {
    if (!(error instanceof Error)) throw error;
    const whodisBadRequestError = findWhodisBadRequestErrorInAxiosError({ axiosError: error as AxiosError });
